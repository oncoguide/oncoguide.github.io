...
- **Optional no-PII notification (opt-in, never automatic):** the ping fires ONLY as part of
  ...
  `{type: sponsorship|form230|donation, locale}` (no name, no amount, no client timestamp --
  receipt time is recorded server-side anyway; less data avoids weak re-identification when
  cross-referenced with an inbound email) -- so the association knows to expect a copy. The supporter is told this ping contains no personal data. The actual document
...
Gemini score: 10 (PASS, 2026-06-11; noted defects fixed in-place) | Implementation: DONE 2026-06-12 (workflow agent + full test suite: 28/28 fill-harness asserts incl. mirrors/checkboxes/overlay coords, 31/31 DOM-harness asserts incl. gate, a11y, consent gating, zero pings on generate). Documented deviations: filename `Formular-230-OncoGuide.pdf` WITHOUT {nameSlug} (the donor name is never collected -- the form has no name field, D6/v7 data minimization); privacy link uses the page's real localized slug `/ro/confidentialitatea-donatorilor/`. Page-2 policy resolved: the annex (page 2) is for additional beneficiaries only -- mirrors filled, NO entity overlay, matching the validated `.private` reference. Fable implementation review >9.3 pending.
...
Gemini score: 9.8 (PASS, 2026-06-11; noted defects fixed in-place) | Implementation: DONE 2026-06-12 (workflow agent + tests: 51/51 contract harness incl. calculator edge cases + amount-in-words, 26/26 F177 harness, 12/12 template anonymity asserts; audit checks (a)-(c) verified live against the template). Appendix B refinements recorded below. Engine note: fillContract gained a strict checkbox fallback (`v === true` -> check) so art5_a/art5_b reach the contract checkboxes; T4/T5 paths regression-tested unchanged. Fable implementation review >9.3 pending.
...
| `suma` | form state | sponsorship amount (RON, numeric) |
| `suma_litere` | form state | amount in words (the vetted contract has the "(...lei)" blank; generated by the flow's roWordsLei helper) -- added in T6 |
| `data_contract` | form state | contract date |
| `art5_varianta` | form state | implemented in T6 as TWO checkboxes `art5_a` / `art5_b` (the vetted Art. 5 has two [ ] options); the flow checks exactly one |
...
| `beneficiar_reg163` | `association.yaml` | filled only when `registru163_no` is set; the Art. 1 blank was reworded in T6 to "...deduceri fiscale: [field]" so the full `registru163_no` value reads grammatically |
